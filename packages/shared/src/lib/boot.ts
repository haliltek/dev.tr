...
const enrichBootWithFeatures = async (boot: Boot): Promise<Boot> => {
  ...
  if (!process.env.NEXT_PUBLIC_EXPERIMENTATION_KEY) {
    let features: Record<string, FeatureDefinition> = {};
    if (boot.exp.features) {
      features = boot.exp.features;
    } else if (boot.exp.f && typeof boot.exp.f === 'string' && boot.exp.f.startsWith('{')) {
      try {
        features = JSON.parse(boot.exp.f);
      } catch {
        features = {};
      }
    }
    return { ...boot, exp: { ...boot.exp, features } };
  }
