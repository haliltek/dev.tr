...
import { useFeedScope } from '../components/layout/FeedScopeToggle';
...
const isTurkishContent = (title?: string): boolean => {
  if (!title) return false;
  // 1. Turkish-specific characters
  if (/[çğıöşüÇĞİÖŞÜ]/.test(title)) return true;
  // 2. Common Turkish tech & grammar keywords
  return /\b(ve|ile|veya|icin|için|nasil|nasıl|nedir|ne|bir|bu|da|de|gibi|kadar|en|son|yeni|hakkinda|hakkında|uzerine|üzerine|rehberi|dersi|mimarisi|kullanimi|kullanımı|kurulumu|incelemesi|gelistirici|geliştirici|yazilim|yazılım|yapay|zeka|otomasyon|sistemleri|cozum|çözüm|test|surum|sürüm|teknoloji|turkce|türkçe|deneyim|egitim|eğitim|komutlari|komutları|yonetimi|yönetimi|karsilastirmasi|karşılaştırması|analizi|araci|aracı|verisi|tablosu|kullanarak|birlikte)\b/i.test(
    title,
  );
};
